    ...
    fn data(&mut self) -> Option<impl Iterator<Item = Datum<'_>>> {
        self.stats.as_mut().map(|s| {
            ...
            if let Some(p) = self.points.as_mut() {
                ...
                stats_data.insert(stats_data.len(), Datum::int("生命值", &mut p.hp, 0, i32::MAX));
                ...
                stats_data
                    .insert(stats_data.len(), Datum::int("最大生命值", &mut p.max_hp, 0, i32::MAX));
                stats_data
                    .insert(stats_data.len(), Datum::int("最大专注值", &mut p.max_fp, 0, i32::MAX));
                stats_data.insert(
                    stats_data.len(),
                    Datum::int("最大精力", &mut p.max_stamina, 0, i32::MAX),
                );
            }
            if let Some(b) = self.blessings.as_mut() {
                stats_data.insert(stats_data.len(), Datum::separator());
                stats_data.append(&mut vec![
                    Datum::byte("幽影树庇护", &mut b.scadutree, 0, 20),
                    Datum::byte("灵灰庇护", &mut b.revered_spirit_ash, 0, 10),
                ]);
            }
